...
class Toast extends EventEmitter {
  ...
  #winrt(xmlString, keepalive){
    ...
    if (this.eventNames().length > 0){
      toast.on("activated", async(_, event) => {
        clearTimeout(timer);
        const eventArgs = winRT.notifications.ToastActivatedEventArgs.castFrom(event); //cast to ToastActivatedEventArgs (OpaqueWrapper)

        //temporary module load
        const Foundation = (await import("@nodert-win10-20h1/windows.foundation")).default;
        const Collection = (await import("@nodert-win10-20h1/windows.foundation.collections")).default;
        
        const valueSet = Collection.ValueSet.castFrom(eventArgs.userInput); //cast to ValueSet (OpaqueWrapper)
        ...
        {
          const value = Foundation.IPropertyValue.castFrom(iterator.current.value); //cast to IPropertyValue (OpaqueWrapper)
