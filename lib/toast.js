  ...
  #winrt(xmlString, keepalive){
    ...
    if (this.eventNames().length > 0){
      toast.on("activated", (_, event) => {
        clearTimeout(timer);
        const eventArgs = winRT.notifications.ToastActivatedEventArgs.castFrom(event); //cast to ToastActivatedEventArgs 
        
        /*user input
        not sure how to get the values from the <OpaqueWrapper> (nodeRT)
        */
        //eventArgs.UserInput.values.toList()[0].toString()
        console.log( eventArgs?.userInput);
        //Windows.Foundation.Collections.ValueSet
        //@nodert-x/windows.foundation.collections ?
        //relevant: https://github.com/NodeRT/NodeRT/issues/119
        
        this.emit("activated", eventArgs?.arguments); 
      });
