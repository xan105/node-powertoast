...
const scenarios = [
  ...
  "reminder", 
  "incomingCall", 
  "urgent" //win11 insider Preview Build >= 22546
];
...
function normalize(option) {
  ...
  let options = {
    ...
    time: isIntegerPositive(option.time) ? option.time * 1000 : Date.now(), //Unix time in ms
    expiration: isIntegerPositive(option.expiration) ? Date.now() + (option.expiration * 60 * 60 * 1000) : null //number of hours from now until discarded
  };
